...
@interface MyRuHuWeiXiuView ()

@end

@implementation MyRuHuWeiXiuView
...
- (void)loadData
{
    NSString *url = [NSMutableString stringWithFormat:@"%@%@?APPKey=%@&userid=%@", api_base_url, api_getmyweixiu, appkey, [[UserModel Instance] getUserValueForKey:@"id"]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([comArray count] > 0)
    {
        if (indexPath.row < [comArray count])
        {
            ...
            if (![com.reply_time isEqualToString:@"0"]) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (complain) {
        MyRuHuWeiXiuDetailView *weixiuDetail = [[MyRuHuWeiXiuDetailView alloc] init];
        weixiuDetail.complain = complain;
        [self.navigationController pushViewController:weixiuDetail animated:YES];
    }
