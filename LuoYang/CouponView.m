...
- (void)loadData
{
    ...
    NSMutableString *url = [NSMutableString stringWithFormat:@"%@%@?APPKey=%@&bid=%@", api_base_url, api_shopscoupons, appkey, typeId];
    ...
    if ([couponArray count] > 0)
    {
        if (indexPath.row < [couponArray count])
        {
            ...
            if ([typeId isEqualToString:@"-1"]) {
                ...
                cell.titleLb.text = coupon.title;
            }
            else
            {
                cell.storenameLb.text = coupon.title;
                cell.titleLb.text = coupon.summary;
            }
